...
is silent, or an incorporated authority speaks only for its own value
domain, and this package's value model decides), or **engine** (a refusal
...
records a question this package has not yet settled; the stub's current
text states the current answer and says that it is current, not settled.
...
| `$string` of an integer that is a float64 in the reference | `$string(1234567890123456789)` is `"1234567890123456800"`; `$string(1e21)` is `"1e+21"`; `$string(1e23)` is `"1e+23"` | the exact digits: `"1234567890123456789"`; `"1000000000000000000000"`; `"99999999999999991611392"` | value model, overriding the incorporated rendering for a value that is an integer in this model: `JSON.stringify` speaks for binary64, and these values are integers here |
...
| Integers above 2^53 | binary64 throughout: `9007199254740993 = 9007199254740992` is true; `9007199254740993 - 9007199254740992` is `0`; `9007199254740993 / 3` is a float | exact: false; `1`; the exact integer `3002399751580331`; `$string` yields the digits | value model |
| An integral float64 beyond 2^53 in arithmetic | float arithmetic: `1e23 + 1` is `1e23`; `1e300 * 1e300` is D1001 | the exact integer the float64 denotes: `99999999999999991611393`; the exact 2000-bit product, within `MaxIntegerBits` | value model: a value is an integer when it is integral, whatever its representation |
| `$power` with integer operands beyond float64 | `$power(2, 64)` is `18446744073709552000`; `$power(10, 400)` is an error (D3061) | exact `18446744073709551616`; exact within `MaxIntegerBits`; D3061 is never raised | value model |
| Mixed integer and decimal arithmetic beyond 2^53 | rounds silently | error E1008 (`CodeInexact`) | engine; refuse rather than approximate; whether this refusal or a single rounding is the rule is pending |
...
| Ill-formed strings | `JSON.parse` accepts `"\ud800"` and JavaScript strings may hold unpaired surrogates | `Unmarshal` refuses the escape and invalid UTF-8 text (E1006); a string that is not valid UTF-8 is refused when observed or marshaled (E1001) | engine; refuse rather than coerce |
...
| Regular-expression dialect | JavaScript | pending ruling | pending: whether the documentation's regular-expression page names the JavaScript syntax (incorporated) or describes constructs without naming a dialect (silent) decides the authority class; the recommendation on record is Go's `regexp`, declared, with unsupported constructs refused at Compile |
